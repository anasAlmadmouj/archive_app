...
// It then sets the document in the 'usersCollection' collection with the userId as the document ID and the user's information as the document data.
  ...
  UsersModel? selectedUsers = UsersModel(
      userName: ' users',
      userId: '-1',
      departmentId: '-1',
      userTypeId: '-1',
    ...
    await FireStoreStorage.fireStore
      ...
      for (var element in value.docs) {
        checkSubCategoryList
            .add(SubCategoryModel.fromJson(element.data()).subCategoryNameAr);
      }
  ...
  Future<bool> checkIsCategoryAvailable({
    ...
    await FireStoreStorage.fireStore
      ...
      for (var element in value.docs) {
        checkCategoryList
            .add(CategoryModel.fromJson(element.data()).categoryNameAr);
      }
  ...
  Future<bool> checkIsTermAvailable({
    ...
    await FireStoreStorage.fireStore
      ...
      for (var element in value.docs) {
        checkTermList
            .add(AcademicTermsModel.fromJson(element.data()).academicTermsNameAr);
      }
  ...
  void usersOnTap({required int index ,required BuildContext context}){
    maybePop(context);
    changeUser(usersModel: userList[index]);
  }
}
